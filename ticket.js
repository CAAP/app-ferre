	...
	ferre.addFuns = function addFuns() {
		...
		req.onerror = function(e) {  console.log('Error loading database: ' + k.DB + ' | ' + e.target.errorCode); };
	        req.onsuccess = function(e) { k.CONN = e.target.result; k.LOAD(); }; //if (k.DB == 'ticket') { loadTICKET(); } };
	    ...
	    var newItem = function newItem(a) {
		var row = ans.insertRow();
	    ...
	    DATA.LOAD = function loadDATA() {
		readDB( DATA ).index( DATA.INDEX ).openCursor().onsuccess = function(e) {
		    var cursor = e.target.result;
		    if (cursor) {
			newItem( cursor.value );
			cursor.continue();
		    }
		};
	    };
	    ...
	    var findIt = function findIt( k ) { var child = ans.children[k]; child.scrollIntoView(); child.focus(); };

	    var searchByDesc = function searchByDesc(s) {
		console.log('Searching by description:' + s);
		var range = IDBKeyRange.lowerBound(s);
		readDB( DATA ).index( DATA.INDEX ).openCursor( range, 'next' ).onsuccess = function(e) { findIt( e.target.result.value ); };
	    };
		...
		req.onsuccess = function(e) {
		    if (e.target.result) { findIt( e.target.result.k ); }
		    else { searchByDesc(s); }
		};
	    };
