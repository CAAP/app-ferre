...
local function fromWeek(week, vers)
	...
	return format('[%s]', concat(data, '\n')) -- XXX decision EITHER send an array or process this DOWN
    else
	return false -- :empty
    end
end
...
-- XXX changes this accordingly XXX
-- find all updates that need to be sent to a specific peer & send them all
local function adjust(msgr, fruit, week, vers)
    fd.reduce(stream(week, vers), function(s) if s then msgr:send_msg(format('%s update %s', fruit, s)) end end)
    return 'Updates sent'
end
...
--	XXX process
	print( adjust(msgr, o.fruit, o.week, o.vers), '\n' )
